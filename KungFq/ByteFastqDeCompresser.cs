...
namespace KungFq
{
    public class ByteFastqDeCompresser : IFastqDeCompresser
    {

        public ByteFastqDeCompresser(int l)
        {
            length = l;
        }
        ...
        void EncodeSeq(byte[] buffer, ref long i, BinaryWriter writer, FastqReader reader)
        {
            ...
            if (b == BUFFER && !lastByte) {
                ...
                //first += (byte) 64; //we have to tell the decoder that we have a block with a length
        ...
        public void Decompress(EncodedSequenceReader sequenceReader, EncodedQualityReader qualityReader, StreamWriter writer)
        {
            ...
            while (sequenceReader.HasSeqLeft(seqI, 1) || continueSequenceRunLength != 0 || continueSequenceChar[0] != ' ' ||
                   qualityReader.HasQLeft(qualI, 1)  || continueQualityRunLength != 0) {
				Spin("Decoding...");
				Console.Error.WriteLine("Big loop");
                q = 0;
                s = 0;
                while (s < length) {
					Console.Error.WriteLine("seq Big loop");
                    if (continueSequenceRunLength != 0) {
                    ...
                    } else if (sequenceReader.HasSeqLeft(seqI, 1)) {
                        encoded = sequenceReader.GetSeqByte(seqI);
						Console.Error.WriteLine("For {0} got {1}", seqI, encoded);
                        seqI++;
                        if ((encoded & 128) == 128) { //run length
							...
							Console.Error.WriteLine("gettubg {0}", encoded);
                            string triplet = decoding[(int) encoded];
							Console.Error.WriteLine(triplet + " AA " + writer + " " + triplet.Length);
                            int k = 0;
                            while (s < length && k < 3) {
						...
						Console.Error.WriteLine("sdasdA");
						break;
					}
                }
                nSeq++;
                ...
                while (q < length) {
                    if (continueQualityRunLength != 0) {
                    ...
                    } else if (qualityReader.HasQLeft(qualI*8, 1)) {
                        encoded = qualityReader.GetQualBByte(qualI);
                        qualI++;
                        if ((encoded & 128) != 128) { //single quality data
                            ...
                            encoded = (byte) (127 & encoded);
                            which = Convert.ToChar(encoded);
                            howmany = (int) qualityReader.GetQualBByte(qualI);
                            qualI++;
                            int i = 0;
                ...
                if (sequenceReader.HasSeqLeft(seqI, 1) || continueSequenceRunLength != 0) {
                    ...
                    //not a padding
                    writer.WriteLine("\n@ID." + nSeq);
                } else if (!sequenceReader.HasSeqLeft(seqI, 1) && continueSequenceChar[0] != ' ')
                    break;
            }
            writer.WriteLine();
			Console.Error.WriteLine();
        ...
        /*static string Decode(byte encoded)
        {
            if ( (encoded & 128) == 128) { //run length
                encoded = (byte) (127 & encoded);
                int howmany =  (encoded >> 3) + 4;
                char which = Bases[(int) (encoded & 7)];
                return "".PadLeft(howmany, which);
            } else { //three bases
                return decoding[(int) encoded];
            }
        }*/

        /* Array that stores correspondences between triplets and their binary encoding. */
        static ByteFastqDeCompresser() {
            decoding[1] = "AAA";
            decoding[2] = "AAC";
            decoding[3] = "AAG";
            decoding[4] = "AAT";
