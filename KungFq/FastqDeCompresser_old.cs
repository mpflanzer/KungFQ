...
namespace KungFq
{
    public class FastqDeCompresser : IFastqDeCompresser
    {
        ...
        const int BUFFER = 1048574; // -1 because of first. 1 Mb blocks
        ...
        char[] progress = {'|', '/', '-', '\\'};
        int spinCounter = -1;
        byte[] continuationSeqBuffer = new byte[BUFFER];
        int continuationSeqLength = 0;
        ...
        public void Compress(FastqReader reader, BinaryWriter writer)
        {
            ...
            while (true) {
                //Spin("Encoding...");
                //Console.Error.WriteLine("Encoding read n.{0}", idCounter);
                ...
                if ((seqI <= qualI || !encodeQualities) && reader.HasSeqLeft(seqI, 1)) {
                    EncodeSeq(writer, ref seqI, reader);
                    continue;
                }
            ...
            //Console.Error.WriteLine("Written {0} {1} {2}", idCounter, seqI, qualI);
        }
        ...
        void ChooseIddc(EncodedFastqReader reader) {
            ...
            if (iddcType == 0) {
                ...
                throw new Exception("Missing header info in compressed file!");
            }
        }
         ...
         */
        void EncodeSeq(BinaryWriter writer, ref long i, FastqReader reader)
        {
            byte first = (byte) 128;
            WriteBitShepherd memoryBits = new WriteBitShepherd(new BinaryWriter(new MemoryStream(BUFFER+4)));
            //XXX
            int byteContinuation = 0;
            int b = BUFFER;
            
            int writtenBits = 0;
            //continuation, always smaller than BIT_BUFFER
            while (continuationSeqLength > 7) {
                //Console.Error.WriteLine("Marking continuation {0}", continuationSeqLength);
                memoryBits.Write(continuationSeqBuffer[byteContinuation], 8);
                continuationSeqLength -= 8;
                writtenBits += 8;
                byteContinuation++;
            }
            if (continuationSeqLength != 0) {
                //Console.Error.WriteLine("Marking continuation2 {0}", continuationSeqLength);
                memoryBits.WriteHighestBits(continuationSeqBuffer[byteContinuation], continuationSeqLength);    
                //Console.Error.WriteLine("Marking continuation {0}", continuationSeqBuffer[byteContinuation]);
                writtenBits += continuationSeqLength;
                continuationSeqLength = 0;
            }
                ...
                if (reader.GetSeq(i) == reader.GetSeq(i+3) && reader.GetSeq(i) == reader.GetSeq(i+2) 
                 && reader.GetSeq(i) == reader.GetSeq(i+1)) {
                    ...
                    while (j < l && reader.HasSeqLeft(j, 1) && reader.GetSeq(j) == reader.GetSeq(j-1))
                        j++;
                    int length = (int) (j - i);
                    //Console.Error.WriteLine("Seen rl {0}", length);
                    if (length > 19) { 
                        //memoryBits.Write(127, 7); //flag for long run length
                        memoryBits.Write(63, 6); //flag for long run length
                        memoryBits.Write(Array.IndexOf(Bases, reader.GetSeq(j-1)), 3);
                        memoryBits.Write(length-4, 8);
                        writtenBits += 17;
                    } else {
                        //memoryBits.Write(0, 7); //flag for short run length
                        memoryBits.Write(1, 6); //flag for short run length
                        memoryBits.Write(Array.IndexOf(Bases, reader.GetSeq(j-1)), 3);
                        memoryBits.Write(length-4, 4);
                        writtenBits += 13;
                    }
                    i = j;
                }
                    ...
                    if (frequent) {
                        ...
                        memoryBits.Write(encoded, 6);   
                        writtenBits += 6;
                    } else {
                        //Console.Error.WriteLine("Econdign rare {0}",encoded);
                        memoryBits.Write(0, 6);
                        memoryBits.Write(encoded, 7);   
                        writtenBits += 13;
                    }
                    //memoryBits.Write(Encode(reader.GetSeq(i), reader.GetSeq(i+1), reader.GetSeq(i+2)), 7);
                    //Console.Error.WriteLine("Written triplet {0} {1}",  Encode(reader.GetSeq(i), reader.GetSeq(i+1), reader.GetSeq(i+2)), writtenBits);
                    i += 3;
                    //writtenBits += 7;
                }
            }
            ...
            if (last != "") {
                ...
                //memoryBits.Write(Encode(last[0], last[1], last[2]), 7);
                //writtenBits += 7;
            }
            if (end) {
                memoryBits.Write(62, 6); // mark end of sequences blocks
                writtenBits += 6;
                first += 32;
            }
            
            if (writtenBits != BIT_BUFFER) {
                b = (int) writtenBits / 8;    
                if (writtenBits % 8 != 0)
                    b++;
            }
            ...
            memoryBits.Flush();            
            //now memoryBits could have 0, 1, 2 or 3 byte more than BUFFER
            
            //bad performance issues TODO
            if (writtenBits > BIT_BUFFER) {
                continuationSeqLength = writtenBits - BIT_BUFFER; //continuation bits
                int howManyBytes = ((int) continuationSeqLength / 8) + 1;
                //Console.Error.WriteLine("cipciap {0} {1} {2} {3} ", whereToStart, howManyBytes, writtenBits,  BIT_BUFFER);
                memoryBits.Writer.BaseStream.Position = BUFFER;
                memoryBits.Writer.BaseStream.Read(continuationSeqBuffer, 0, howManyBytes);
                memoryBits.Writer.BaseStream.Position = 0;
                writer.Write(first);
                //Console.Error.WriteLine("AAA {0} - {1} {2}", howManyBytes, continuationSeqBuffer[0], writtenBits);
                //Console.Error.WriteLine("SEQAAAAAAAAAA0 written buffer {0} - {1} {2} {3}", first, writtenBits, b, writer.BaseStream.Length);
                writer.Write(((MemoryStream) memoryBits.Writer.BaseStream).GetBuffer(), 0, BUFFER);
                //Console.Error.WriteLine("Writing for seq as first buffer byte {0}", ((MemoryStream) memoryBits.Writer.BaseStream).GetBuffer()[0]);
                //Console.Error.WriteLine("SEQ0 written buffer {0} - {1} {2} {3}", first, writtenBits, b, writer.BaseStream.Length);
            } else if (writtenBits == BIT_BUFFER) {
                memoryBits.Writer.BaseStream.Position = 0;
                memoryBits.Writer.BaseStream.Read(continuationSeqBuffer, 0, BUFFER);
                writer.Write(first);
                writer.Write(continuationSeqBuffer, 0, BUFFER);
                //Console.Error.WriteLine("SEQ3 written buffer {0} - {1} {2} {3} {4}", first, b,  writer.BaseStream.Length, continuationSeqBuffer[0], continuationSeqBuffer[1]);
            } else {
                memoryBits.Writer.BaseStream.Position = 0;
                memoryBits.Writer.BaseStream.Read(continuationSeqBuffer, 0, b);
                //for (int m = 0; m < b; m++)
                //    Console.Error.WriteLine(continuationSeqBuffer[m]);
                writer.Write(first);
                writer.Write(b);
                writer.Write(continuationSeqBuffer, 0, b);
                //Console.Error.WriteLine("SEQ written buffer {0} - {1} {2} {3}", first, b,  writer.BaseStream.Length, continuationSeqBuffer[0]);
                //if (b >= 2)
                //    Console.Error.WriteLine(continuationSeqBuffer[1]);
            }
            memoryBits.Close();
        }
        ...
        void EncodeQual(byte[] buffer, ref long i, BinaryWriter writer, FastqReader reader)
        {
            ...
            if (b == BUFFER) {
                writer.Write(first);
                writer.Write(buffer);
                //Console.Error.WriteLine("0QUAL written buffer {1} - {0} {2}", first, BUFFER, writer.BaseStream.Length);
            } else {
                ...
                //Console.Error.WriteLine("QUAL written buffer {1} - {0}, {2} {3}", first, b, buffer[0], writer.BaseStream.Length);
            }
        }
        
        private void CheckIDNeed(int s, int nSeq, StreamWriter writer, ref long IdByte, EncodedFastqReader reader)
        {
            if (!encodeIds && s == 0) {
                if (nSeq == 0) {
                    writer.WriteLine("@{0}", nSeq);    
                }
                else {
                    writer.WriteLine("\n@{0}", nSeq);
                }
            } else if (s == 0) {
                String id = iddc.GetNextID(ref IdByte);
                /*if (id.Length == 0) {
                    endFastq = true;
                    return;
                }*/
        ...
        public void Decompress(EncodedFastqReader reader, StreamWriter writer)
        {
            ...
            while (hasAlready7SeqBits 
                   || (hasAlready7SeqBits = reader.GetSeqBitsAsInt(ref sevenSeqBits, ref seqI, 6))
                   || continueSequenceRunLength != 0 || continueSequenceChar[0] != ' ' 
                   || reader.HasQLeft(qualI, 1)  || continueQualityRunLength != 0) {
                q = 0;
                s = 0;
                int encodedInt = 0;
        
                while (s < length) {
                    //Console.Error.WriteLine("In the seq main loop {0} {1} {2} {3} {4}", q, s, hasAlready7SeqBits,
                    //                        continueSequenceRunLength, continueSequenceChar[0], sevenSeqBits, encodedInt);
                    if (!hasAlready7SeqBits && reader.GetSeqBitsAsInt(ref sevenSeqBits, ref seqI, 6)) {
                        hasAlready7SeqBits = true;
                    }
                    if (continueSequenceRunLength != 0) {
                        //Console.Error.WriteLine("in the first else");
                        CheckIDNeed(s, nSeq, writer, ref IdByte, reader);
                        //if (endFastq)
                        //    break;
                    ...
                    } else if (hasAlready7SeqBits || reader.GetSeqBitsAsInt(ref encodedInt, ref seqI, 6)) {
                        ...
                        if (encodedInt != 1 && encodedInt != 63 && encodedInt != 62) { //triplet 
                            ...
                            if (encodedInt == 0) { //rare triplet or end 
                                ...
                                reader.GetSeqBitsAsInt(ref encodedInt, ref seqI, 7);
                            ...
                            if (encodedInt == 63) //long rl
                                rlLengthBits = 8;
                            reader.GetSeqBitsAsInt(ref encodedInt, ref seqI, 3);
                            which = Bases[encodedInt];
                            reader.GetSeqBitsAsInt(ref howmany, ref seqI, rlLengthBits);
                            howmany += 4;
                            int i = 0;
                            CheckIDNeed(s, nSeq, writer, ref IdByte, reader);
                ...
                while (encodeQualities && q < length) {
                    //Console.Error.WriteLine("In the qual main loop {0} {1} {2}", q, s, hasAlready7SeqBits);
                    if (q == 0 && (continueQualityRunLength != 0 || reader.HasQLeft(qualI, 1)))
                        writer.WriteLine("\n+");
                    if (continueQualityRunLength != 0) {
                        ...
                        //Console.Error.WriteLine("quality {0}", encoded);
                        qualI++;
                        if ((encoded & 128) != 128) { //single quality data
                            which = Convert.ToChar(encoded);
                            writer.Write(which);
                            //Console.Error.WriteLine("single quality {0}", which);
                            q++;
                        } else { //run length
                            ...
                            qualI++;
                            int i = 0;
                            //Console.Error.WriteLine("rl {0} {1}", which, howmany);
